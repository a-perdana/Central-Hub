...
| `cambridge_scheme_of_work/{docId}`      | Cambridge scheme-of-work content per ref code: teaching activities, learning objectives, external resources, SDG links. **Doc ID format: `{subjectCode}_{code}` e.g. `0580_C1.1`** — same convention as `cambridge_syllabus`. Fields: `subjectCode`, `code`, `tier` (`'core'`/`'extended'`, lowercase), `title`, `topicArea`, `learningObjectives[]`, `teachingActivities[{body, tags[]}]` (tags: `I`/`E`/`F`/SDG-N), `resources[{title, url, type}]` (type: `'external'`/`'resource_plus'`/`'past_paper'`), `sdgLinks[{goals[], suggestion}]`, `syllabusVersion`, `schemeOfWorkVersion`, `examFromYear`, `examToYear`. Surfaced via Teaching Guide tab on the objectives modal in Teachers Hub pacing pages. Seeded by `seed-scheme-of-work-{code}.js` from `cambridge-scheme-of-work-{code}.json`. **Math 0580 seeded (125 entries); biology/chemistry/physics not yet seeded.** | central_admin |
