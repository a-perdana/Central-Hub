...
| `induction_programs/{programId}` | 39 handbook docs across 5 `handbookKind` values, seeded from `docs/induction/handbook-*.json` (6 induction tracks) + `docs/handbooks/**` (33). **Source of truth is JSON; Firestore docs are reverted on next seed.** | central_admin via seed |
...
**Operations:** `school-appraisals`, `teacher-appraisals`, `activities` (kanban, sidebar dropped 2026-05-25, canonical My Work hero + board pills row), `school-visits`, `kpi-admin`, `reports`. **Moved to Curriculum:** `assessment-management` → "Pacing Assessments" (lives under Curriculum > Pacing Assessments since 2026-05-11). **Retired pages (2026-05-27, physically removed):** the standalone `appraisals.html` Coming-Soon stub, the `assessments.html` redirect stub, the `ease-system.html` 4-cycle EASE handbook, and the orphaned `settings.html` are all gone — `appraisals` inbound links now route to `school-appraisals`.
...
**Admin tooling:** `console`, `page-access`, `competency-admin`, `induction-admin`, `orientation-admin`, `checklist-admin`, `schedule-settings`, `feedback-management`, `rules-viewer`, `design-system`, `weekly-checklist`, `diagrams` (per-subject diagram repository)
...
**References & Standards (2026-05-09, expanded 2026-05-14):** `references` — single HQ surface for every framework / audit / handbook / Cambridge standard / Indonesian regulation. Multi-facet · search + `?doc=<id>` deep-link + localStorage MRU-5 recently-viewed. The **"Handbooks" facet** loads ALL handbooks live from Firestore `induction_programs` (`limit(50)`) — 30 docs across 5 `handbookKind` values — the same query the `/handbook` reader uses, so the two surfaces stay in sync by construction (counts computed at runtime, not hardcoded). The static-doc manifest (frameworks / Cambridge / Permendiknas / ES / AICF / schemas) is in `references.html`; it must match the files under `dist/references-data/` — 3 dead entries (archived `specialist-content-*` v1 JSONs) were pruned 2026-06-26 to keep manifest↔files in sync. References-data lives in `dist/references-data/` (build copies from monorepo `docs/` + AH/CH `resources/`). AH and TH `/references` fetch this cross-origin (CORS-open). Deep-links handbook cards into `/handbook?id=<programId>` reader.
