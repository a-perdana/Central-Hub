/**
 * One-time upload script: loads IGCSE-Physics.json into igcse_syllabus Firestore collection.
 *
 * Usage:
 *   node upload-igcse-physics-syllabus.js
 *
 * Requirements:
 ...
 *   e.g. igcse_syllabus/0625_1.1_core
 *
...
const JSON_PATH    = path.join(__dirname, 'resources/IGCSE-Physics.json');
const SUBJECT_CODE = '0625';
...
if (!fs.existsSync(JSON_PATH)) {
  console.error('IGCSE-Physics.json not found at:', JSON_PATH);
  process.exit(1);
}
...
async function run() {
  ...
  for (const entry of data) {
    ...
    const docId   = `${SUBJECT_CODE}_${code}_${tier}`;                     // e.g. "0625_1.1_core"
  ...
  console.log(`Done. Uploaded ${count} physics syllabus entries to igcse_syllabus.`);
  process.exit(0);
}
