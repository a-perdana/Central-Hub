...
| `ch_phase[]` (CH) | subset of `primary`, `secondary`, `early_years`. Grade-band scope of an Eduversal Specialist, added 2026-08-23. Orthogonal to `ch_subjects[]` — the Academic Board runs separate Specialists for Mathematics (Primary) and Mathematics (Secondary) and both carry `ch_subjects: ['math']`, so the subject enum alone cannot tell them apart. **Empty or absent means every phase**, which keeps every pre-existing profile at the access it already had. Enforced in CH `auth-guard.js` (`SUBJECT_PAGE_PHASES` + `isPhaseAllowed`); `central_admin` and `director` bypass. Canonical enum in `Central Hub/partials/subject-config.js` `PHASES`. | console.html |
| `appraisal_level` (TH teacher) | `induction` \| `developing` \| `proficient` \| `lead`. Decides which F2 Class Observation items are in scope — 30 items at Induction, 37 Developing, 44 Proficient, 45 Lead (`visible_from` per item in `appraisal-framework-v2.json`). Read by CH `/teacher-appraisal-entry` + AH `/teacher-appraisal-entry`; **absent/empty is scored as Induction**. Companion audit fields `appraisal_level_setAt` + `appraisal_level_setBy →users.uid`. | CH `/teacher-levels` (central_admin) |
...
`appraisal_2026_27` (added 2026-07-27) — the school's supervision plan for the appraisal year, seeded from the *Updated Appraisal Model: Overview* §2.1/§3.1–3.3 by [`scripts/appraisal/seed-school-supervisors-2026-27.js`](../../scripts/appraisal/seed-school-supervisors-2026-27.js). Shape: `{primarySupervisor, secondarySupervisor, visit1{dates, team[]}, visit2{dates, team[]}, source, updatedAt}`. Supervisors are **role labels** ("Physics Subject Specialist", "Math SS Primary"), not uids — the model names positions, so staff turnover does not invalidate the plan. Labels name the **appraisal seat**, i.e. the subject specialty; three of the ten experts also hold a directorship, and that title belongs on `/coordinators-directory`, never here. The Primary Supervisor signs the Phase 1 report; the Secondary signs Phase 3. CH `/school-appraisals` reads this to show the supervision strip and to name the signees on the composed report. The seed script asserts the model's own invariants before writing: 15 schools, every expert supervising exactly 3, and every supervisor present on the visit team they sign for.

**Admin-editable since 2026-07-27.** The ✏️ button on each `/school-appraisals` row opens a Supervision Plan modal (`central_admin` only — `partner_schools` write is `isAHAdmin()`) that edits both supervisors, both visit date strings and both five-person teams. Two things follow from the plan being stored **on the school, not on the appraisal record** it is opened from: (1) an edit applies to *every* appraisal of that school for the year, and the modal states this with a live count of affected records; (2) **re-running the seed script overwrites hand edits** — manual saves stamp `source: 'manual-edit-school-appraisals'` (plus `updatedBy`) so a post-seed diff is explicable rather than mysterious. The modal re-checks the model's supervisor-attended-the-visit invariant and warns once before allowing an override, rather than blocking: the seed asserts it, but a legitimate mid-year cover arrangement should not be unrepresentable. **Team array position IS the domain mapping** (`defaultDomainPic()` seeds the per-domain lead by index), so the editor preserves interior blanks and only drops trailing ones.
...
22 collections following the same shape: `{subject}_pacing/{yearKey}` (one doc per year-cohort).
...
| `bahasa_pacing` `checkpoint_bahasa_pacing` `primary_bahasa_pacing` `asalevel_bahasa_pacing` | (per page) | `central_admin` / CH coordinator / `bahasa` specialist (+ `teachers_admin` on non-IGCSE) |
| `edu_steam_pacing` `checkpoint_edu_steam_pacing` `primary_edu_steam_pacing` `asalevel_edu_steam_pacing` | (per page) | `central_admin` / CH coordinator / `edu_steam` specialist (+ `teachers_admin` on non-IGCSE) |

**Doc shape:** `chapters[]`, `classes[]`, `objPrefixes[]`, `weeklyHours`, plus per-topic week/hour metadata.
**Read:** any authorised user.

**Eduversal-authored non-Cambridge subjects (2026-05-31):** Bahasa Indonesia (`*bahasa_pacing`) and Edu-STEAM (`*edu_steam_pacing`) mirror the Cambridge pacing structure across all 4 stages (Primary / Checkpoint / IGCSE-Secondary / AS-A-Level) but carry no Cambridge syllabus code — the pacing pages show an **EDV-prefix placeholder** (`EDV-BAH-P/C/S/A`, `EDV-STM-P/C/S/A`) and the `cambridge_syllabus` autocomplete returns no matches until HQ authors a real scheme. The IGCSE-tier docs use the bare `bahasa_pacing` / `edu_steam_pacing` names (matching `subject-config.js` SUBJECT_PACING_LINKS slugs); other stages prefix as usual.

**Subject scoping (Step 10, 2026-05-03):** Each `*_pacing` collection's write rule restricts subject specialists to their own subject via the `isCHSubjectSpecialist(subjects)` helper. A central_user with `ch_subjects: ['math']` can write `math_pacing` / `asalevel_math_pacing` / `checkpoint_math_pacing` / `primary_math_pacing` but NOT biology / chemistry / etc. Combined-science pacing (`checkpoint_science_pacing`, `primary_science_pacing`) accepts any of biology / chemistry / physics specialists. Bahasa / Edu-STEAM pacing accepts the `bahasa` / `edu_steam` specialty respectively (single-subject). central_admin and CH coordinators (`ch_sub_roles: ['coordinator']`) bypass the subject scope entirely.
...
**PK:** auto-id.
**Fields:** `schoolId →partner_schools.id`, `schoolName`, `academicYear` (dash format, e.g. `2026-2027`), `appraiser`, `status` (`draft` / `submitted` / `reviewed`), `statusHistory[]`, `ratings{}`, `evidence{}`, `files{}`, `links{}`, `strengths`, `improvements`, `recommendations`, `externalReview{phases{}, report{}, frameworkVersion}`, `createdAt`, `updatedAt`, `createdBy`, `updatedBy`.

**`statusHistory[]` — the audit trail on `status` (2026-07-28).** Every transition appends one row: `{from, to, at (ISO string), by (email), byName, action ('advance'|'revert'), via ('academichub'|'centralhub')}`. Appended with `arrayUnion` from both hubs — AH stamps the school's own `draft → submitted`, CH stamps `submitted → reviewed` and every reopen. Before this the record kept **no** trace of a status move: `updatedBy` holds only the last writer of *any* field and is overwritten by the next ordinary save, so neither "who submitted this" nor "who reopened it, and how many times" was answerable. `action: 'revert'` marks a backward move specifically, because reopening retracts a declaration someone else made — CH `/school-appraisals` renders the reopen count in the modal footer beside the button that produces it. **ISO strings, not `serverTimestamp()`** — Firestore rejects a sentinel inside an array (same reason `actionPlan.submittedAt` is an ISO string). Order is by the `at` field, not array position: two hubs append to the same array and `arrayUnion` guarantees no ordering. Append-only by convention, not by rule — branch (a) admin and branch (b) same-school AH writes are both unclamped, so a client could rewrite the array; it is an operational record, not tamper-proof evidence. Records predating this field have no trail and degrade to showing nothing.

**Reopening a submitted or signed-off record (2026-07-28).** AH disables every input the moment `status` leaves `draft`, and CH's Save deliberately never writes `status` — so before this a school that submitted by mistake was stuck until someone edited Firestore by hand. CH `/school-appraisals` now carries an admin-only **Return to school** button, active on both `submitted` and `reviewed`, writing `status: 'draft'`. It arms on first click and fires on the second (`alert`/`confirm` are banned — CH Common Mistake #3), and the confirm label names what is being undone when the record was already `reviewed`. No rule change was needed: branch (a) already granted `central_admin` every field. Note this softens the "sign-off is a single accountable act" reading of `reviewed` — the accountability now rests on `statusHistory[]` recording the retraction rather than on the state being terminal.

**`externalReview.phases{}` — one slice per visit.** The Updated Appraisal Model (AY 2026–2027) sends a team to each school **twice**, 56–63 days apart, and the Annual Report summarises both. So Eduversal's findings are keyed by phase: `phases.phase_1` (Teaching Quality Visit) and `phases.phase_3` (Validation Visit) carry `{ratings{}, justifications{}, outcome, outcomeNote, visitDates, evaluatedBy, evaluatedAt, contributors[]}`; `phases.phase_2` (Progress Check-in) is online and carries the same shape **minus ratings** — re-scoring 33 aspects over a video call would be a fiction, so it records an outcome and notes only.

**`contributors[]` — everyone who saved the phase (2026-07-28).** `evaluatedBy` is the *last* writer and it overwrites, so on a visit run by five domain leads the four who wrote first vanished from the record. `contributors[]` keeps one row per person — `{email, name, firstAt, lastAt, saves}` — appended by `saveAppraisal()` on every save, deduped **on email** so a display-name change never splits one person into two rows. Oldest first. `evaluatedBy` / `evaluatedAt` stay as the last-writer stamp; the byline on `/school-appraisals` renders the list and falls back to the single name on records written before this field existed. Rides inside `phases{}`, so it needs no rule change (the `hasOnly` clamp is on `externalReview`'s top-level keys).
...
**`appraiser` is retired (2026-07-27) — read-only legacy, never written by CH.** It predates the supervision model: it held whatever name someone typed into the New Appraisal form. The supervision pair on `partner_schools.appraisal_2026_27` now answers "who appraises this school", so a second field holding a name could only contradict it. CH `/school-appraisals` **no longer writes the field on create** and the Appraiser picker is gone from the New Appraisal form; the only remaining read is the legacy fallback in `supervisorLine()`, which renders the typed name in italic for records made before the plan existed. **Do not reintroduce a write path.** One loose end: AH [`SchoolSelfAppraisal.html`](../../Academic%20Hub/SchoolSelfAppraisal.html) still writes `appraiser: profile.displayName` when a school self-creates its record — that value is the *school user's* own name, never an appraiser, and it is what the italic fallback renders for AH-created rows. Drop it there on the next AH touch. The create rule requires no field presence, so removing it is rule-safe.

Each rating phase also records **who attended**: `team[]` and `domainPic{<domainId>: roleTag}` as role tags (`"Physics Subject Specialist"`), plus `teamConfirmed` / `teamConfirmedBy` / `teamConfirmedAt`. `partner_schools.appraisal_2026_27` says who was *scheduled*; this says what happened, because cover is routine — someone falls ill and a colleague goes. Without it an appeal cannot be reviewed with the person who actually saw the domain being challenged (model §8.1), and cross-network calibration has no author to attribute a rating to. Pre-filled from the plan, correctable, then confirmed.
...
**Publishing the report is gated on the same conditions (2026-07-28).** `reportReadiness()` in CH `/school-appraisals` blocks **Publish to school** until *both* rating phases have every domain fully rated, a confirmed team, and a person recorded against each rated domain. Previously the panel checked only `canWriteExternal && !published`, so a record with one rated aspect and no confirmed team could be published; `nextStep()` sequenced the work but never blocked it. Anchored to ES 17.8 — the written report carries a rating per domain, and the year-end School Development Rating is the mean of all five, so a phase that skipped a domain can supply neither. Team confirmation rides along because an appeal is reviewed with whoever saw that domain (model §8.1). **Drafting stays open** — a first pass against a partial visit is useful and the draft is private until published; only the irreversible step is gated. UI-level only, like the justification crosswalk above: the rule layer still accepts the write.
...
**`appeals{cases{}, decisions{}}` — challenging a domain rating.** Updated Appraisal Model §8.1: a school with 10 working days from the report may appeal a domain rating it believes the evidence does not support; Eduversal reviews with the relevant domain lead and responds within 15 working days, and the decision is final. Both maps are keyed by **domain id** (`d1` … `d5`) — one appeal per domain. A case is `{grounds, submittedBy, submittedOn}`; a decision is `{outcome ('upheld'|'not_upheld'), note, reviewedWith, decidedBy, decidedOn}`.

Same two-way ownership as the action plan, and here it is the whole point: rule branch (g) lets the school write `cases` while requiring `decisions` back byte-equal — **a school can never write its own outcome**; branch (h) lets `central_admin` write `decisions` while requiring `cases` back byte-equal, so the grounds cannot be edited to fit the decision. Deciding is `central_admin` only, not any `central_user`: the model gives this to the CAO and calls the decision final, so it goes through the narrowest door available. Both hubs compute the deadline in **working days**, not calendar days — a test asserts the two implementations agree.
...
`html` is sanitised against a shared tag allowlist on every write in CH **and** again on render in AH — no `<script>`, `<iframe>`, `<a>` or `<img>`, and every attribute stripped except Quill's `ql-align-*` classes. Keep the two allowlists identical; a test asserts they match. **`report.status` — not the record's `status` — decides whether the school sees it:** AH `/school-self-appraisal` renders it read-only only once `'published'`. That is a presentation rule, not a confidentiality boundary — `read` on this collection is open to any academic user, so a draft is technically fetchable. Do not put anything in a draft that would harm the school if read early.
...
#### `specialist_observations/{docId}` (Eduversal Specialist Lesson Observation)
**PK:** auto-id. Written from CH `/teacher-appraisal-entry`.
**Fields:** `teacherUid →users.uid`, `observerUid →users.uid`, `observerName`, `observerRole` (`'subject_specialist'`), `observerHub` (`'centralhub'`), `schoolId →partner_schools.id`, `school`, `subject`, `level`, `academicYear`, `blocks[]` (always `['F2']`), `contributes_to_formal_score` (always `false`), `observation{date, classGroup, topic}`, `scores{itemId→1-4|'na'}`, `notes{itemId→string}`, `f2Mean`, `narrative{glow, grow, go}`, `status` (`'draft' | 'shared'`), `createdAt`, `updatedAt`.
**Writers:** `central_user` stamping themselves as `observerUid`; observer can update own while not finalised; `central_admin` deletes.
**Read:** the teacher, any central_user, AH admin, TH admin, `central_admin`, and same-school AH leadership (`schoolId` match) so a school can see Eduversal's observation of its own staff.

**Deliberately separate from `teacher_appraisals`** (decided 2026-07-28). A specialist observation is Eduversal's own quality lens on teaching across the network, scored against the F2 rubric only. The school's formal appraisal — the record that carries `compositePercent` + `predicate` — is owned by the school's leadership and lives in `teacher_appraisals`, written from AH `/teacher-appraisal-entry`. Merging them made a partial F2 row indistinguishable from a complete four-block appraisal to `/teacher-appraisals` and `/reports`, which count rows without checking any partial flag. Same separation principle as `induction_observations` vs appraisal (Charter NN1). **Do not wire `specialist_observations` into composite/predicate calculation, the AH end-of-year "load observation record" lookup, or appraisal-completion reporting.**
...
**Subcollection:** `competency_framework/{trackId}/levels/{compId}_{level}` — per-(competency, level) base learning-path content. Doc ID format `{compId}_{level}` e.g. `smc-1_awareness`, `evsi-1_practitioner`. Fields: `trackId`, `compId`, `level`, `reading` (long-form prose ~200-400 words; v2 entries target 800-1500 chars, ESL CEFR-B2), `keyTakeaways[]`, `selfAssessment[]` (5 evidence-anchored self-rating statements), `activity{task, output, duration, evidence}`, **`source`** (`'hand_authored_v1'` for legacy entries; `'hand_authored_v2'` for Specialist v2 refresh entries written 2026-05-19), **`v2RefreshNote`** (Specialist v2 marker — present iff source == 'hand_authored_v2'), `authoredAt`, `updatedAt`. Lazy-fetched by learning-path pages on modal open. Admin overrides remain in `content_overrides_{teachers,academic}/{compId}_{lvl}` and are layered on top of this base. Seeded by `scripts/competency/seed-competency-content.js` (TH + AH v1) or `scripts/competency/seed-specialist-content-v2.js` (Specialist v2). **Specialist track is the only one currently passing all v2 quality thresholds** — Indonesia density ≥ 2.0/entry, freshness density ≥ 0.50/entry, pedagogy density ≥ 0.30/entry (audit script: `scripts/competency/audit-specialist-content.js`).
...
- `sourceP` is upstream's observed correct-rate. **Layer 1 (2026-05-31)** turned it into a real difficulty prior: `scripts/ease/rebootstrap-difficulty-from-sourcep.js` writes `sourcePLogit = -ln(p/(1-p))` (p clamped [0.02, 0.98]) and the EASE runner now prefers it over the 3-band `DIFF_LOGIT` bootstrap (preference: `calibratedLogit` > `sourcePLogit` > band). Applied to math + science first (7,097 items); the author bands were near-indistinguishable (mean p 0.70/0.68/0.66) so the prior is a strict improvement to early-window item selection. The weekly `calibrateEaseItems` Cloud Function still supersedes it with `calibratedLogit` once `seenCount ≥ 30`. See `docs/students-hub/EASE-BANK-ANALYSIS-2026-05-31.md`.
- **Layer 2 — AI Cambridge tagging (2026-05-31).** `scripts/ease/ai-tag-cambridge.js` (offline batch, Anthropic `claude-sonnet-4-6`, ANTHROPIC_API_KEY from env, admin SDK) sends batches of items + the candidate `cambridge_syllabus` objective list for that subject+stage and writes an `aiTagSuggestion` PROPOSAL + `tagStatus:'proposed'`. The model sees only stem text (≤240 chars) + difficulty + cognitiveTag — never image URLs or answers (mirrors `practiceBankAiSuggest` privacy posture, Common Mistake #39). `strandCode` is derived from the matched objective's Cambridge `topicArea`, not invented. HQ confirms/rejects in CH `/ease-item-author` (preview-modal panel + row "✨ AI tag · review" chip + tag-status filter); confirm copies the proposal onto live `cambridgeStandardRefs[]`/`strandCode`. Scope: math (0862) + science (0893) first; **English (0861) is NOT in `cambridge_syllabus` yet** (Gap D) so English tagging is deferred. No rule change — `ease_items` update already allows admin + CH coordinator. Indexes added: `tagStatus+createdAt`, `subjectId+tagStatus+createdAt`.
...
- Academic Board roster (2026-08-23): `academicBoardMember` (boolean) and `domainLead` (`'d1' | 'd2' | 'd3' | 'd4' | 'd5' | null`) — the seat this Specialist holds on the School Appraisal. This collection is the roster host: before this the ten-member Board and its Domain Lead pairing lived only in an org-chart graphic and the Appraisal Module's internal guide, so nothing could check ES 24.2's "each domain is owned by two Domain Leads" against data. The Board member's seniority rank (Director / Consultant / Coordinator) is carried by `entryKind`.
