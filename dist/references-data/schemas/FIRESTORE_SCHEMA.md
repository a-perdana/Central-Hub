...
`externalReview.report` is the written report ES 17.8 requires within 10 business days of the visit: `{status ('draft'|'published'), summary, domains{<domainId>: string}, commendations, recommendations, priorities, generatedAt, publishedAt, publishedBy}`. It is auto-drafted in CH `/school-appraisals` from the ratings, the deltas against the school's self-appraisal and the justifications behind them, then edited by hand before publishing. **`report.status` — not the record's `status` — decides whether the school sees it:** AH `/school-self-appraisal` renders it read-only only once `'published'`. That is a presentation rule, not a confidentiality boundary — `read` on this collection is open to any academic user, so a draft is technically fetchable. Do not put anything in a draft that would harm the school if read early.
