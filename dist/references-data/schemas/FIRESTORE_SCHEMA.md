...
**⚠ `appraiser` is a vestigial field — decide its fate before the next cycle.** It predates the supervision model: it held whatever name someone typed into the New Appraisal form. Since 2026-07-27 that form picks an **expert role** from the framework roster, and the table and modal read the supervision pair from `partner_schools.appraisal_2026_27` instead. So the field is still *written* on create but no longer *read*, except as a legacy fallback for records made before the plan existed (rendered in italic). Two honest options: retire it, or redefine it as "who opened this record" and rename accordingly. Leaving it as-is invites someone to trust a value nothing maintains.
