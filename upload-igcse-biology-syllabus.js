/**
 * One-time upload script: loads IGCSE-Biology.json into igcse_syllabus Firestore collection.
 *
 * Usage:
 *   node upload-igcse-biology-syllabus.js
 *
 * Requirements:
 ...
 *   e.g. igcse_syllabus/0610_1.1_core
 *
...
const JSON_PATH    = path.join(__dirname, 'resources/IGCSE-Biology.json');
const SUBJECT_CODE = '0610';
...
if (!fs.existsSync(JSON_PATH)) {
  console.error('IGCSE-Biology.json not found at:', JSON_PATH);
  process.exit(1);
}
...
async function run() {
  ...
  for (const entry of data) {
    ...
    const docId   = `${SUBJECT_CODE}_${code}_${tier}`;                     // e.g. "0610_1.1_core"
  ...
  console.log(`Done. Uploaded ${count} biology syllabus entries to igcse_syllabus.`);
  process.exit(0);
}
